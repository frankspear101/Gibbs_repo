...
! $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
! $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
	Subroutine AutoTan(logFileOn,iGrid,ierr)
    	use AWE_Interfaces
	use MyCanvas
...
!		if(iremove.eq.4)call DumpEverything()			
		...
		do j = maxNegativeMolesPhase,numPh
...
!		Check to see if any phases are below the tangent plane
		...
		do k = 1,numPhMIF
			if(gDifferenceAU(k).lt.-1.0d-4)then			
				...
				!write(*,*)'		   Phase lies below tangent = ',k
...
!c*******************************************************
!c*******************************************************
	Subroutine ResetToStart()
!	Routine to set the assemblage to the LAST good one
	implicit none
! *****************************************
	include "Assemb.inc"
	include "Tangent.inc"
...
!      local variables
	...
	numCurrent = numCurrentLast
	do i = 1,numCurrent
		asmCurrent(i) = asmCurrentLast(i)		! set the assemblage to the last good one
		end do
...
!      local variables
	...
	numCurrent = numCurrentNewP
	do i = 1,numCurrent
		asmCurrent(i) = asmCurrentNewP(i)		! set the assemblage to the last good one
		end do

	do i = 1,numPhMIF
		MP0(i) = MPNewP(i)
		do j = 1,numPhCo(i)
			xPhCo(i,j) = xPhCoNewP(i,j)
			end do
		end do
	return
	end
...
!      local variables
	...
	numCurrentLast = numCurrent
	do i = 1,numCurrent
		asmCurrentLast(i) = asmCurrent(i)		! set the assemblage to the last good one
		end do

	do i = 1,numPhMIF
		MPLast(i) = MP0(i)
		do j = 1,numPhCo(i)
			xPhCoLast(i,j) = xPhCo(i,j)
			end do
		end do
...
!      local variables
	...
	numCurrentNewP = numCurrent
	do i = 1,numCurrent
		asmCurrentNewP(i) = asmCurrent(i)		! set the assemblage to the last good one
		end do

	do i = 1,numPhMIF
		MPNewP(i) = MP0(i)
		do j = 1,numPhCo(i)
			xPhCoNewP(i,j) = xPhCo(i,j)
			end do
		end do
	return
	end
...
!      local variables
	integer*4 i,j,k
	real*8 molesSyComp,molesFluidToChange,molesFluidNew,molesChange,sum
...
! --------------------------------------------------------------------
!      local variables
	integer*4 i,j,k,ifrac,kcur
	real*8 molesSyComp,molesChange,weightChange,sum


!	Fractionation code
!	Check to see if any of the fractionating phases are part of the current assemblage
	do i = 1,nc
		MeltWt(i) = 0.0d0		! zero out meltWt so that it will be 0 unless extracted
		end do

	MeltMoles = 0.0d0		! zero out so it will be 0 unless extracted
	MeltVol   = 0.0d0
	do 4473 iFrac = 1,numFractionate
	removeVol(iFrac) = 0.0d0
	removeMoles(iFrac) = 0.0d0
	do 4472 kcur = 1,numCurrent
	k = asmCurrent(kcur)
	if(toFractionate(iFrac).eq.k)then
		!write(*,*)TC4,PB4
!		write(*,*)numFractionate,toFractionate(iFrac),residual(ifrac),threshold(ifrac)
!		write(*,*)phName(k),mode(k)
!		pause 'hit return'
!		See if the volume% (mode) is over the threshold
		if(mode(k).ge.threshold(iFrac))then
!			Calculate the amount of this phase to remove
			removeVol(iFrac) = mode(k) - residual(iFrac)
!			write(*,*)removeVol
!			pause 'remove mode ---hit return'
!			This part is tricky
!				The threshold and residual are based on modal amounts (easier to envision for user0
!				However, the volumes (VP0) and moles (MP0) used in the calculations are based on the total grams of the input bulk composition
!				The conversion factor is the quantity "sum", which is the total volume of the phases in the assemblage
!				and used to calculate modes in Subroutine CalculateMode(sum)
			removeVol(iFrac) = removeVol(iFrac)*sum/100.0d0		! this is the volume to remove based on initial bulk composition
			removeMoles(iFrac) = removeVol(iFrac)/(vp0(k)/mp0(k))		! this is the moles of the phase to remove
!			write(*,*)sum,removeVol,removeMoles
!			pause 'sum,removevol,removemoles --- hit return'
			if(removeMoles(iFrac).le.0.0d0)then
				removeMoles(iFrac) = 0.0d0		! do not allow phase to be added back into the rock
				go to 4473		
				endif
!			write(*,*) 'comp, molesinPhase,moleschange,BCold,BCnew --- hit return'
			do i = 1,nc
				molesSyComp = 0.0d0
	!			Remove moles of each system component for this phase
				do j = 1,numPhCo(k)
					molesSyComp = molesSyComp + xPhCo(k,j)*comp(k,j,i)	! amount of the system component in the phase		
					end do
				molesChange = molesSyComp*removeMoles(iFrac)			! amount of system component to remove
				if(trim(PhName(k)).eq.'Melt')then	!calculate the melt composition in grams
					weightChange = molesChange*molwt(i)/NumCatInOxide(i)
					MeltWt(i) = weightChange
					meltMoles = MP0(K)
					meltVol = VP0(k)
					endif
	!			molesChange = molesSyComp*molesFluidToChange
	!			write(*,*)i,coname(i),molesSyComp,MolesChange,bulkCompMoles(i),bulkCompMoles(i) - molesChange
				bulkCompMoles(i) = bulkCompMoles(i) - molesChange		!bulkCompositionMoles is the same as molesStart, but used elsewhere (I should consolidate these)
				molesStart(i) = bulkCompMoles(i)				!molesStart is the bulk comp we want to fit in C2ompute
				end do
			do i=1,nc
...
!			Calling here only puts out the BC after it changes
!			The same calls are made above, which puts out the BC at every T and P
!			call WriteBCMoles			! unit=75
!			call WriteBCWt				! unit = 76
			endif
		if(trim(PhName(k)).eq.'Melt')then	!calculate the melt composition in grams
			call WriteMeltWt
			endif
		endif
4472	continue	! loop on current assemblage
4473	continue	! loop on phases to fractionate
	call WriteFractionate
	return
	end
! $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
! $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
! $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
	Subroutine AutoTanIsothermal(logFileOn,iGrid,ierr)
    	use AWE_Interfaces
	use MyCanvas

!	This routine is designed to calculate the stable phase assemblage for a specified bulk composition 
!	It is exactly the same as Subroutine AutoTan except this routine iterates at constant T rather than constant P
!	Because of this (a) fractionation and (b) constant porosity are not implemented
!
!	We use the same routines "SetNewP" and "ResetToNewP" even though we're actually resetting to newT (uses same codes)
...
!      local variables
	...
	integer*4 i,j,iTP,iok,k,izero,isubtend,maxNegativeMolesPhase,maxgDifferencePhase,iTried
	integer*4 asmCurrentStore(phMax),kCur,newT,countTheMisses
	real*4 Tc4,Pb4,TC4old,TCLast,xplot,yplot
	real*8 maxgDifference,maxNegativeMoles,gDiffThreshold
	real*8 sum
	integer*4 logFileOn,iGrid,iremove
	integer*4 ierr,idumped
...
!	if(Dabs(PB4-PB4old).gt.1.)then		! this should print every time P changes - just to show progress
	if(Dabs(TC4-TC4old).gt.0.1)then		! this should print every time P changes - just to show progress
!		If we are holding the porosity constant, then we need to reset the H2O content to have it in excess here
!		if(constantPorosity.eq.0.and.numFractionate.eq.0)then
!			write(12,*)TC4,PB4,(asmCurrent(iok),iok=1,numCurrent)
			write(*,*)TC4,PB4
...
! 			endif
	...
	if(newT.eq.1)then
!		If we are starting the next pressure, use the results from the previous P at the same T as a starting point
		call ResetToNewP()		! this just resets to the last calculate T at the first P
!		call DumpEverything()
		else
!		Otherwise, use the last assemblage as the starting point
		call ResetToLast()
...
!	So, if compositions are off for some reason, the code fails due to the activity calculation (not the uzero calculation)
	...
	!	call DumpEverything()
		...
		close(74)	! close ALL file
		close(80)	! close ELM file
		close(81)	! close OXI file
		return
		endif
	countTheMisses = 0
410	continue
...
!		endif
	...
	call SetLast()
	if(newT.eq.1)then		! We have a solution for the initial T for the new pressure - save
		call SetNewP()
		endif
400	continue			! loop back to get the next temperature
	close(74)	! close ALL file
...
! $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
! $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
	Subroutine AutoTanOSFract(logFileOn,iGrid,ierr,kGrt,mGrt,MgrtTotal,Radius)
    	use AWE_Interfaces
	use MyCanvas
...
!      local variables
	...
	integer*4 i,j,iTP,iok,k,izero,isubtend,maxNegativeMolesPhase,maxgDifferencePhase,iTried,Kgrt
	integer*4 asmCurrentStore(phMax),kCur,newP,countTheMisses
	real*4 Tc4,Pb4,Pb4old,TCLast,xplot,yplot
	real*8 maxgDifference,maxNegativeMoles,gDiffThreshold,Mgrt,MgrtTotal,Radius
	real*8 sum
	integer*4 logFileOn,iGrid,iremove
	integer*4 ierr,idumped
	character*16 TPtext
...
!	write(12,45766)(molesStart(i),i=1,nc),(phComp(kGrt,i),i=1,nc)
45766	format(40E15.7)
	write(95,45704)TC4,PB4,(molesStart(i)*1000,i=1,nc)
45704	format(2F12.2,20F12.5)
	write(95,45703)(molwt(i)*molesStart(i)/numCatInOxide(i),i=1,nc)
45703	format(24x,20F12.5)
	write(95,45703)(xPhCo(kGrt,j),j=1,numPhCo(kGrt))
